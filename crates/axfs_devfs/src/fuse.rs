...
impl VfsNodeOps for FuseDev {
    ...
    fn read_at(&self, offset: u64, buf: &mut [u8]) -> VfsResult<usize> {
        info!("fuse_dev read buf len: {:?} at pos: {:?}", buf.len(), offset);
        ...
        // buf.fill(0);
        let fusein = fuse_in_header::new(104, 16, 37, 2, 0, 0, 0, 0);
        fusein.write_to(buf);
        let initin = fuse_init_in::new(10, 229, 0, 0, 0, 0, 0, 0, 0, 0);
        initin.write_to(buf[40..]);
